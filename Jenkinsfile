pipeline {
    ...
    environment {
        ...
        ECR_URL = '211125460791.dkr.ecr.eu-central-1.amazonaws.com/peja-result-repo'
    }
    tools {
        dockerTool 'docker'
    }
    stages {
        ...
        stage('Docker Config and Build Image') {
            steps {
                sh 'sudo docker build -t peja-master-infra-result .'
            }
        }
        stage('Push to ECR') {
            steps {
                script {
                    if (env.BRANCH_NAME == 'main') {
                        ...
                        sh "sudo docker tag peja-master-infra-result ${ECR_URL}:latest"
                        sh "sudo docker tag peja-master-infra-result ${ECR_URL}:${version}"
